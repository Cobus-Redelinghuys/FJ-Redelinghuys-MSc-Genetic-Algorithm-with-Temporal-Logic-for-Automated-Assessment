    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < 14; i++) {
            ...
            if (studentMap.containsKey(moduleName)) {
                ...
                jsonObject.put("stdOut", studentMap.get(moduleName).get("stdout"));
                jsonObject.put("errOut", studentMap.get(moduleName).get("stderr"));
                jsonObject.put("exeTime", studentMap.get(moduleName).get("duration"));
                jsonObject.put("exitCode", studentMap.get(moduleName).get("exitvalue"));
                /*
                 * jsonObject.put("instructorStdOut",
                 * instructionMap.get(moduleName).get("stdout"));
                 * jsonObject.put("instructorErrOut",
                 * instructionMap.get(moduleName).get("stderr"));
                 * jsonObject.put("instructorExeTime",
                 * instructionMap.get(moduleName).get("duration"));
                 * jsonObject.put("instructorExitCode",
                 * instructionMap.get(moduleName).get("exitvalue"));
                 */
