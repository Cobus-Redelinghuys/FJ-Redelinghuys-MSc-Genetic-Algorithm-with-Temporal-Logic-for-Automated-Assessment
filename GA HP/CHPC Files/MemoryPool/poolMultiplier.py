...
for i in range(1, 11):
    ...
    with open(target_dir+"/MemoryPool.job", "r") as f:
        lines = f.readlines()
        
    lines[5] = "#PBS -o /mnt/lustre/users/fredelinghuys/Pools/MemoryPool/Pool{}/MemoryPool{}.out\n".format(i,i)
    lines[6] = "#PBS -e /mnt/lustre/users/fredelinghuys/Pools/MemoryPool/Pool{}/MemoryPool{}.err\n".format(i,i)
    lines[11] = "cd /mnt/lustre/users/fredelinghuys/Pools/MemoryPool/Pool{}/\n".format(i)
    
    with open(target_dir+"/MemoryPool.job", "w") as f:
        f.writelines(lines)
        
    os.rename(target_dir+"/MemoryPool.job", target_dir+"/MemoryPool{}.job".format(i))
